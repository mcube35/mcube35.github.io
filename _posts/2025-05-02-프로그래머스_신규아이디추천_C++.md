...
* [4, 6단계]
    - `stack` 에 담았던 첫 글자가 `.` 이라면 요소들 전체를 이동시켜 첫번째 문자를 삭제
    - `stack` 의 크기가 15를 넘는다면 15로 리사이징
    - 리사이징후에 마지막 문자가 `.` 이라면 `stack` 에서 삭제
...
* [5, 7단계 + 마무리]
    - `stack` 의 크기가 0이면 'a' 문자 담기
    - `stack` 의 크기가 3보다 작다면 크기가 3이 될때까지
      마지막 문자를 계속해서 넣어주기
